  ...
  async execute(interaction) {
    ...
    try {
      ...
      return await interaction.editReply(
        "Error fetching community goals. Please try again later.",
      );
    }
  },
};
...
function processData(data) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setFooter({
      text: "Data provided by INARA, click title to view on their site",
    });
  ...
  if (!communityGoals || communityGoals.length === 0) {
    embed.setDescription(
      "Community goal data was not retrieved. Ensure you have the correct INARA API key and that the API is available.",
    );
    return embed;
  }
  ...
  if (activeGoals.length === 0) {
    embed.setDescription(
      "There are currently no active community goals in Elite Dangerous.",
    );
    return embed;
  }
  ...
  activeGoals.forEach((goal, index) => {
    ...
    embed.addFields({
      ...
      value:
        `**Location:** ${goalSystem} - ${goalStation}\n` +
